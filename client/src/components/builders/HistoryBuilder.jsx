  ...
  <div className="space-y-5">
    <ControlSection subtitle="Choose how historical bookings are presented." title="Layout">
      <SelectField
        ...
        value={config.layout}
      />
      <ColorField label="Surface color" onChange={(surfaceColor) => updateSection({ surfaceColor })} value={config.surfaceColor} />
      <ColorField label="Text color" onChange={(textColor) => updateSection({ textColor })} value={config.textColor} />
      <ColorField label="Header color" onChange={(headerColor) => updateSection({ headerColor })} value={config.headerColor} />
    </ControlSection>
  </div>
);
