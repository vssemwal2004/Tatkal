...
const DashboardPreview = ({ config, project, routes = [] }) => {
  ...
  const labels = {
    fromLabel: resolveLabel(config.fromLabel, 'From', 'City', isEvent),
    toLabel: resolveLabel(config.toLabel, 'To', 'Venue', isEvent),
    buttonLabel: resolveLabel(config.buttonLabel, 'Search routes', 'Search events', isEvent),
    itemConnector: isEvent ? '•' : 'to',
    itemLabel: isEvent ? 'tickets left' : 'seats left'
  };
  const previewStats = isEvent
    ? [
        { label: 'Live Events', value: '32' },
        { label: 'Daily Bookings', value: '1.4k' },
          ...
          <div className="p-6" style={{ color: config.textColor }}>
            ...
            >
              <div className="grid gap-4 md:grid-cols-3">
                <SearchField label={labels.fromLabel} value="Ahmedabad" />
                <SearchField label={labels.toLabel} value="Mumbai" />
                <button
                  className="rounded-[22px] px-4 py-4 text-sm font-semibold text-white"
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <p className="text-sm font-semibold text-slate-950">
                          {route.from} {labels.itemConnector} {route.to}
                        </p>
                      ...
                      <div className="text-right">
                        ...
                        <p className="mt-1 text-xs text-slate-500">
                          {route.seatsLeft} {labels.itemLabel}
                        </p>
                      </div>
                    </div>
                  </div>
...
const resolveLabel = (value, travelDefault, eventDefault, isEvent) => {
  if (!isEvent) {
