  ...
  return (
    <section className="space-y-6 fade-rise">
      <PageHeader
        ...
        title="Deployments"
        description="Every deployment is listed here with the generated site URL and access details."
      />
            ...
            {deployments.map((item, index) => (
              <tr key={`${item.clientId}-${index}`} className="border-t border-slate-800 text-slate-200">
                ...
                <td className="px-4 py-3">
                  <StatusBadge value={item.businessType} />
                </td>
                <td className="px-4 py-3 text-slate-300">{item.url}</td>
                <td className="px-4 py-3 text-slate-300">{item.username || 'Unavailable'}</td>
                <td className="px-4 py-3">
