...
const businessCards = [
  {
    ...
    description: 'Perfect for routes, seats, dashboard search, and transport checkout journeys.',
    icon: BusFront
  },
  {
    id: 'event',
    title: 'Event Booking',
    description: 'Uses the same structured page flow with ticket-zone styling and event-friendly labels.',
    icon: CalendarDays
  }
];
      ...
      <main className="client-shell">
        <div className="grid gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
          <section className="client-card p-5">
            ...
            <h1 className="mt-2 text-2xl font-semibold text-slate-950">Set the basics before you start designing.</h1>

            <div className="mt-6 space-y-4">
              <label className="block space-y-1.5">
                <span className="text-xs font-medium uppercase tracking-[0.18em] text-slate-500">Platform Name</span>
                <input
              ...
              <label className="block space-y-1.5">
                <span className="text-xs font-medium uppercase tracking-[0.18em] text-slate-500">Owner Name</span>
                <input
                  className="field-base"
            ...
            <div className="client-input-card mt-5">
              ...
              <p className="mt-2 break-all font-mono text-sm text-slate-700">{builderState.project.clientId}</p>
            </div>
          ...
          <section>
            ...
            <h2 className="mt-2 text-3xl font-semibold tracking-[-0.03em] text-slate-950">
              Pick the booking experience you want to build.
            </h2>
            <div className="mt-6 grid gap-4 md:grid-cols-2">
              {businessCards.map((card) => {
                ...
                return (
                  <button
                    className={`client-card client-card-hover p-5 text-left ${isSelected ? 'border-brand-200 bg-brand-50/70' : ''}`}
                    key={card.id}
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      {isSelected ? (
                        <span className="rounded-full border border-brand-200 bg-white px-3 py-1 text-[11px] uppercase tracking-[0.22em] text-brand-700">
                          Selected
                        </span>
                      ) : null}
                    </div>
