...
const resolveConfig = (design) => design?.config || {};

const inlineCssFromDist = () => {
...
const buildClientPreviewHtml = ({ client, design }) => {
  const config = resolveConfig(design);
    ...
    <header class="sticky top-0 z-40 border-b border-white/8 bg-slate-950/60 backdrop-blur-xl">
      <div class="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 sm:px-8">
        <div class="flex items-center gap-3">
          <div class="flex h-11 w-11 items-center justify-center rounded-2xl border border-aurora-400/30 bg-aurora-500/10 text-sm font-bold text-aurora-300">
            TK
          </div>
          <div>
