...
const HistoryPreview = ({ config }) => (
  <PreviewFrame title="Order History Preview">
    <div
      className="mx-auto max-w-5xl rounded-[30px] border border-white/10 p-6"
    ...
    >
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <h3 className="mt-2 font-display text-3xl font-bold">Customer bookings</h3>
        </div>
        <div
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {bookings.map((booking) => (
            <div className="rounded-[26px] border border-white/10 bg-black/20 p-5" key={booking.id}>
              ...
              <h4 className="mt-3 font-display text-xl font-bold">{booking.route}</h4>
