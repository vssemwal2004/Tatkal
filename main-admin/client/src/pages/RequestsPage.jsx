...
import { fetchPendingRequests } from '../services/requestService';
  ...
  return (
    <section className="space-y-6 fade-rise">
      <PageHeader
        ...
        description="Open each submission to inspect the builder JSON, approve the request, and assign the correct booking system."
        action={
          <input
            type="text"
            value={query}
      ...
      {!loading && !filteredRequests.length ? (
        <EmptyState
          title={requests.length ? 'No matching requests' : 'No pending requests found'}
          description={
            requests.length
